...
async function BotFunctions(Discord, client, message, args, db, serverinfo, msgInit) {
    msgInit.edit(new Discord.RichEmbed()
        ...
        .addField(':four: ID do canal de Sugestões', `• Atual: *${serverinfo.map('sugestionChannel').value()}*`)
        .addField(':five: ID do canal de Denúncias', `• Atual: *${serverinfo.map('reportsChannel').value()}*`)
        .addField(':six: ID do canal de Changelog', `• Atual: *${serverinfo.map('ChangelogChannel').value()}*`)
        .setFooter("SkyBlack Network ©️ IP: " + serverinfo.map('IP').value(), message.guild.iconURL)
        .setColor(serverinfo.map('color').value()[0])
    )
